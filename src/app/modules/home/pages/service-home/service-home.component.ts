...
export class ServiceHomeComponent implements OnInit, OnChanges {
  ...
  registerChangeUIEditor() {
    this.eventSubscriberUi = this.eventManager
      ...
      .subscribe((_event) => this.loadUITreeData());
  }

  registerChangeDesignEditor() {
    this.eventSubscriberUi = this.eventManager
      .on(EventTypes.editorDesignTreeListModification)
      .subscribe((_event) => this.loadDesignTreeData());
  }
  ...
  viewComponent(item) {
    ...
    if (item.type === "MODEL") {
      ...
      this.router.navigate(["service/lambda"], {
        queryParams: {
          projectUid: item.projectuuid,
          lamdafunctionUuid: item.uuid,
        },
      });
      ...
      if (projectUUID) {
        ...
        this.projectService
          ...
          .subscribe(
            ...
            (_res: HttpErrorResponse) => this.onSaveError()
          );
      }
    }
  ...
  listenConsoleLogChange() {
    this.consoleEventSubscriber = this.eventManager
      ...
      .subscribe((_event) => this.updateLog());
  }
