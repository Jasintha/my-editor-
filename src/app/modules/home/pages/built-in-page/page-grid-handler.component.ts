...
export class PageGridHandlerComponent implements OnChanges, OnDestroy {
  ...
  grid = {
    ...
    rows: [
      {
        containers: [
          { containerCols: 6, widget: { uuid: '', projectUuid: '', pageuuid: '', widgettemplate: '', widgetTitle: '' } },
          { containerCols: 6, widget: { uuid: '', projectUuid: '', pageuuid: '', widgettemplate: '', widgetTitle: '' } },
        ],
      },
      { containers: [{ containerCols: 12, widget: { uuid: '', projectUuid: '', pageuuid: '', widgettemplate: '', widgetTitle: '' } }] },
    ],
  };
  ...
  loadUpdateForm(builtInPage) {
      ...
      this.findWidgetsForPage(this.projectUid, this.currentPage.uuid);
  }
  ...
  addWidgetToGrid(widget: IWidget) {
    this.grid.rows[this.selectedRowIndex].containers[this.selectedContainerIndex].widget.uuid = widget.uuid;
    ...
    for (let i = 0; i < this.grid.rows.length; i++) {
      for (let j = 0; j < this.grid.rows[i].containers.length; j++) {
        for (let k = 0; k < widgetArray.length; k++) {
          if (this.grid.rows[i].containers[j].widget.uuid === widgetArray[k].uuid) {
            this.grid.rows[i].containers[j].widget.widgettemplate = widgetArray[k].widgettemplate;
            this.grid.rows[i].containers[j].widget.widgetTitle = widgetArray[k].widgetTitle;
          }
        }
      }
    }
  ...
  editWidget(widget) {
    ...
    // const url = 'model-page/proj/' + this.projectUid + '/' + this.currentPage.uuid + '/' + widget.uuid + '/widget-layout';
...
//         return false;
  ...
  setWidgetType(widgetTemplate) {
    ...
    this.saveWidget(widgetTemplate);
  }
