...
@Component({
  selector: 'virtuan-grpc-bidirectional-str-node-config',
  templateUrl: './grpc-bidirectional-str-node-config.component.html',
  providers: [{
    provide: NG_VALUE_ACCESSOR,
    useExisting: forwardRef(() => GRPCBidirectionalStrNodeConfigComponent),
    multi: true
  }]
})
export class GRPCBidirectionalStrNodeConfigComponent implements ControlValueAccessor, OnInit, OnDestroy, AfterViewInit {
  ...
  grpcBidirectionalStrNodeConfigFormGroup: FormGroup;
  ...
  constructor(private translate: TranslateService,
    ...
    this.grpcBidirectionalStrNodeConfigFormGroup = this.fb.group({
  ...
  updateForm(){
      ...
      this.grpcBidirectionalStrNodeConfigFormGroup.patchValue({
...
//         this.returnItems.push({ label: dtodropdownLabel, value: returnObj });
  ...
  setDisabledState(isDisabled: boolean): void {
    ...
    if (this.disabled) {
      this.grpcBidirectionalStrNodeConfigFormGroup.disable({emitEvent: false});
    } else {
      this.grpcBidirectionalStrNodeConfigFormGroup.enable({emitEvent: false});
    }
  }
  
  onReturnObjChange(){
    this.configuration.returnObj = this.grpcBidirectionalStrNodeConfigFormGroup.get(['returnObj']).value;
    ...
    if (this.configuration.returnObj && returnRecordType !== 's' && returnRecordType !== 'm') {
      this.grpcBidirectionalStrNodeConfigFormGroup.get('returnRecordType').patchValue('s', { emitEvent: false });
  ...
  onInputObjChange(){
    this.configuration.selectedAPIInputs = this.grpcBidirectionalStrNodeConfigFormGroup.get(['selectedAPIInputs']).value;
    this.updateModel(this.configuration);
  }
...
//           returnObj: returnObj,
    ...
    this.changeSubscription = this.grpcBidirectionalStrNodeConfigFormGroup.get('returnRecordType').valueChanges.subscribe(
        (configuration: any) => {
    ...
    this.changeSubscription = this.grpcBidirectionalStrNodeConfigFormGroup.get('enableSecurity').valueChanges.subscribe(
        (configuration: any) => {
    ...
    /* this.changeSubscription = this.grpcBidirectionalStrNodeConfigFormGroup.get('selectedAPIInputs').valueChanges.subscribe(
        (configuration: any) => {
  ...
  private updateModel(configuration: RuleNodeConfiguration) {
    ...
    if (this.definedConfigComponent || this.grpcBidirectionalStrNodeConfigFormGroup.valid) {
