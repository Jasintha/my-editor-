...
@Component({
  selector: 'virtuan-get-node-config',
  templateUrl: './get-node-config.component.html',
  providers: [{
    provide: NG_VALUE_ACCESSOR,
    useExisting: forwardRef(() => GetNodeConfigComponent),
    multi: true
  }]
})
export class GetNodeConfigComponent implements ControlValueAccessor, OnInit, OnDestroy, AfterViewInit {
...
//   dataSource = new MatTableDataSource(this.ELEMENT_DATA);
  ...
  getNodeConfigFormGroup: FormGroup;
  ...
  constructor(private translate: TranslateService,
    ...
    this.getNodeConfigFormGroup = this.fb.group({
      returnRecordType: ["", Validators.required],
      returnObj: [null, Validators.required],
      paramType: "",
      paramName: "",
      paramDataType: "",
...
//         this.returnItems.push({ label: dtodropdownLabel, value: returnObj });
  ...
  addRow() {
    const paramType = this.getNodeConfigFormGroup.get(['paramType']).value;
    const paramName = this.getNodeConfigFormGroup.get(['paramName']).value;
    const paramDataType = this.getNodeConfigFormGroup.get(['paramDataType']).value;
  ...
  setDisabledState(isDisabled: boolean): void {
    ...
    if (this.disabled) {
      this.getNodeConfigFormGroup.disable({emitEvent: false});
    } else {
      this.getNodeConfigFormGroup.enable({emitEvent: false});
    }
  }
...
//     );
    ...
    this.changeSubscription = this.getNodeConfigFormGroup.get('resourcePath').valueChanges.subscribe(
        (configuration: any) => {
  ...
  private updateModel(configuration: RuleNodeConfiguration) {
    ...
    if (this.definedConfigComponent || this.getNodeConfigFormGroup.valid) {
