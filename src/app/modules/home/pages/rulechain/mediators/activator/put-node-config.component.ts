...
@Component({
  selector: 'virtuan-put-node-config',
  templateUrl: './put-node-config.component.html',
  providers: [{
    provide: NG_VALUE_ACCESSOR,
    useExisting: forwardRef(() => PutNodeConfigComponent),
    multi: true
  }]
})
export class PutNodeConfigComponent implements ControlValueAccessor, OnInit, OnDestroy, AfterViewInit {
...
//   dataSource = new MatTableDataSource(this.ELEMENT_DATA);
  ...
  putNodeConfigFormGroup: FormGroup;
  ...
  constructor(private translate: TranslateService,
    ...
    this.putNodeConfigFormGroup = this.fb.group({
  ...
  updateForm(){
      ...
      if(returnObj && this.returnItems){
        returnObj = this.returnItems.find(x => (x.paramType === this.configuration.returnObj.id) && (x.inputType === this.configuration.returnObj.inputType) && (x.inputName === this.configuration.returnObj.inputName));
      }
...
//         this.returnItems.push({ label: dtodropdownLabel, value: returnObj });
  ...
  addRow() {
    const paramType = this.putNodeConfigFormGroup.get(['paramType']).value;
    const paramName = this.putNodeConfigFormGroup.get(['paramName']).value;
    const paramDataType = this.putNodeConfigFormGroup.get(['paramDataType']).value;
  ...
  setDisabledState(isDisabled: boolean): void {
    ...
    if (this.disabled) {
      this.putNodeConfigFormGroup.disable({emitEvent: false});
    } else {
      this.putNodeConfigFormGroup.enable({emitEvent: false});
    }
  }
...
//           returnObj: returnObj,
    ...
    this.changeSubscription = this.putNodeConfigFormGroup.get('resourcePath').valueChanges.subscribe(
        (configuration: any) => {
  ...
  private updateModel(configuration: RuleNodeConfiguration) {
    ...
    if (this.definedConfigComponent || this.putNodeConfigFormGroup.valid) {
