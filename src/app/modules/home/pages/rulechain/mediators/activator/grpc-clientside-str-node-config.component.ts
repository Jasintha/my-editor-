...
@Component({
  selector: 'virtuan-grpc-clientside-str-node-config',
  templateUrl: './grpc-clientside-str-node-config.component.html',
  providers: [{
    provide: NG_VALUE_ACCESSOR,
    useExisting: forwardRef(() => GRPCClientSideStrNodeConfigComponent),
    multi: true
  }]
})
export class GRPCClientSideStrNodeConfigComponent implements ControlValueAccessor, OnInit, OnDestroy, AfterViewInit {
  ...
  grpcClientSideStrNodeConfigFormGroup: FormGroup;
  ...
  constructor(private translate: TranslateService,
    ...
    this.grpcClientSideStrNodeConfigFormGroup = this.fb.group({
  ...
  updateForm(){
      ...
      this.grpcClientSideStrNodeConfigFormGroup.patchValue({
...
//         this.returnItems.push({ label: dtodropdownLabel, value: returnObj });
  ...
  setDisabledState(isDisabled: boolean): void {
    ...
    if (this.disabled) {
      this.grpcClientSideStrNodeConfigFormGroup.disable({emitEvent: false});
    } else {
      this.grpcClientSideStrNodeConfigFormGroup.enable({emitEvent: false});
    }
  }
  
  onReturnObjChange(){
    this.configuration.returnObj = this.grpcClientSideStrNodeConfigFormGroup.get(['returnObj']).value;
    ...
    if (this.configuration.returnObj && returnRecordType !== 's' && returnRecordType !== 'm') {
      this.grpcClientSideStrNodeConfigFormGroup.get('returnRecordType').patchValue('s', { emitEvent: false });
  ...
  onInputObjChange(){
    this.configuration.selectedAPIInputs = this.grpcClientSideStrNodeConfigFormGroup.get(['selectedAPIInputs']).value;
    this.updateModel(this.configuration);
  }
...
//           returnObj: returnObj,
    ...
    this.changeSubscription = this.grpcClientSideStrNodeConfigFormGroup.get('returnRecordType').valueChanges.subscribe(
        (configuration: any) => {
    ...
    this.changeSubscription = this.grpcClientSideStrNodeConfigFormGroup.get('enableSecurity').valueChanges.subscribe(
        (configuration: any) => {
    ...
    /* this.changeSubscription = this.grpcClientSideStrNodeConfigFormGroup.get('selectedAPIInputs').valueChanges.subscribe(
        (configuration: any) => {
  ...
  private updateModel(configuration: RuleNodeConfiguration) {
    ...
    if (this.definedConfigComponent || this.grpcClientSideStrNodeConfigFormGroup.valid) {
