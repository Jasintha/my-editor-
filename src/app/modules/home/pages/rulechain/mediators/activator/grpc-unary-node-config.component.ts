...
@Component({
  selector: 'virtuan-grpc-unary-node-config',
  templateUrl: './grpc-unary-node-config.component.html',
  providers: [{
    provide: NG_VALUE_ACCESSOR,
    useExisting: forwardRef(() => GRPCUnaryNodeConfigComponent),
    multi: true
  }]
})
export class GRPCUnaryNodeConfigComponent implements ControlValueAccessor, OnInit, OnDestroy, AfterViewInit {
  ...
  grpcUnaryNodeConfigFormGroup: FormGroup;
  ...
  constructor(private translate: TranslateService,
    ...
    this.grpcUnaryNodeConfigFormGroup = this.fb.group({
  ...
  updateForm(){
      ...
      this.grpcUnaryNodeConfigFormGroup.patchValue({
...
//         this.returnItems.push({ label: dtodropdownLabel, value: returnObj });
  ...
  setDisabledState(isDisabled: boolean): void {
    ...
    if (this.disabled) {
      this.grpcUnaryNodeConfigFormGroup.disable({emitEvent: false});
    } else {
      this.grpcUnaryNodeConfigFormGroup.enable({emitEvent: false});
    }
  }
  
  onReturnObjChange(){
    this.configuration.returnObj = this.grpcUnaryNodeConfigFormGroup.get(['returnObj']).value;
    ...
    if (this.configuration.returnObj && returnRecordType !== 's' && returnRecordType !== 'm') {
      this.grpcUnaryNodeConfigFormGroup.get('returnRecordType').patchValue('s', { emitEvent: false });
  ...
  onInputObjChange(){
    this.configuration.selectedAPIInputs = this.grpcUnaryNodeConfigFormGroup.get(['selectedAPIInputs']).value;
    this.updateModel(this.configuration);
  }
...
//           returnObj: returnObj,
    ...
    this.changeSubscription = this.grpcUnaryNodeConfigFormGroup.get('returnRecordType').valueChanges.subscribe(
        (configuration: any) => {
    ...
    this.changeSubscription = this.grpcUnaryNodeConfigFormGroup.get('enableSecurity').valueChanges.subscribe(
        (configuration: any) => {
    ...
    /* this.changeSubscription = this.grpcUnaryNodeConfigFormGroup.get('selectedAPIInputs').valueChanges.subscribe(
        (configuration: any) => {
  ...
  private updateModel(configuration: RuleNodeConfiguration) {
    ...
    if (this.definedConfigComponent || this.grpcUnaryNodeConfigFormGroup.valid) {
