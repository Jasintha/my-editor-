...
@Component({
  ...
  providers: [{
    ...
    useExisting: forwardRef(() => ShopifyOrderNodeConfigComponent),
    multi: true
  }]
})
export class ShopifyOrderNodeConfigComponent implements ControlValueAccessor, OnInit, OnDestroy, AfterViewInit {
  ...
  shopifyOrderNodeConfigFormGroup: FormGroup;
  ...
  constructor(private translate: TranslateService,
    ...
    this.shopifyOrderNodeConfigFormGroup = this.fb.group({
      type: [],
      action: []
    });
  }
  ...
  setDisabledState(isDisabled: boolean): void {
    ...
    if (this.disabled) {
      this.shopifyOrderNodeConfigFormGroup.disable({emitEvent: false});
    } else {
      this.shopifyOrderNodeConfigFormGroup.enable({emitEvent: false});
    }
  }
  ...
  writeValue(value: RuleNodeConfiguration): void {
    ...
    if (this.definedConfigComponent) {
      ...
      this.shopifyOrderNodeConfigFormGroup.patchValue({
        type: this.configuration.connector.type,
        action: this.configuration.connector.action
      });    
      this.changeSubscription = this.shopifyOrderNodeConfigFormGroup.get('type').valueChanges.subscribe(
        (configuration: RuleNodeConfiguration) => {
      ...
      this.changeSubscription = this.shopifyOrderNodeConfigFormGroup.get('action').valueChanges.subscribe(
        (configuration: RuleNodeConfiguration) => {
...
/*
  ...
  */
  ...
  private updateModel(configuration: RuleNodeConfiguration) {
    if (this.definedConfigComponent || this.shopifyOrderNodeConfigFormGroup.valid) {
