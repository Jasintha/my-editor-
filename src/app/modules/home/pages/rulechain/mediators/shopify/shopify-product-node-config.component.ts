...
@Component({
  selector: 'virtuan-shopify-product-node-config',
  templateUrl: './shopify-product-node-config.component.html',
  providers: [{
    provide: NG_VALUE_ACCESSOR,
    useExisting: forwardRef(() => ShopifyProductNodeConfigComponent),
    multi: true
  }]
})
export class ShopifyProductNodeConfigComponent implements ControlValueAccessor, OnInit, OnDestroy, AfterViewInit {
  ...
  shopifyProductNodeConfigFormGroup: FormGroup;
  ...
  constructor(private translate: TranslateService,
    ...
    this.shopifyProductNodeConfigFormGroup = this.fb.group({
      type: [],
      action: []
    });
  }
  ...
  setDisabledState(isDisabled: boolean): void {
    ...
    if (this.disabled) {
      this.shopifyProductNodeConfigFormGroup.disable({emitEvent: false});
    } else {
      this.shopifyProductNodeConfigFormGroup.enable({emitEvent: false});
    }
  }
  ...
  writeValue(value: RuleNodeConfiguration): void {
    ...
    if (this.definedConfigComponent) {
      ...
      this.shopifyProductNodeConfigFormGroup.patchValue({
        type: this.configuration.connector.type,
        action: this.configuration.connector.action
      });    
      this.changeSubscription = this.shopifyProductNodeConfigFormGroup.get('type').valueChanges.subscribe(
        (configuration: RuleNodeConfiguration) => {
      ...
      this.changeSubscription = this.shopifyProductNodeConfigFormGroup.get('action').valueChanges.subscribe(
        (configuration: RuleNodeConfiguration) => {

          this.configuration.connector.action = configuration;
          this.updateModel(this.configuration);
        }
      );
...
/*
  ...
  */
  ...
  private updateModel(configuration: RuleNodeConfiguration) {
    if (this.definedConfigComponent || this.shopifyProductNodeConfigFormGroup.valid) {
