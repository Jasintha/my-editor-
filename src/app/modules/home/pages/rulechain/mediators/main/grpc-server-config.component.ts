...
@Component({
  selector: 'virtuan-grpc-server-node-config',
  templateUrl: './grpc-server-config.component.html',
  providers: [{
    provide: NG_VALUE_ACCESSOR,
    useExisting: forwardRef(() => GRPCServerNodeConfigComponent),
    multi: true
  }]
})
export class GRPCServerNodeConfigComponent implements ControlValueAccessor, OnInit, OnDestroy, AfterViewInit {
  ...
  grpcServerNodeConfigFormGroup: FormGroup;
  ...
  constructor(private translate: TranslateService,
    ...
    this.grpcServerNodeConfigFormGroup = this.fb.group({
      inputType: [],
      param: [],
      constant: [],
      property: [],
  ...
  refreshInputTypes(){
    ...
    let inputType: string = this.grpcServerNodeConfigFormGroup.get('inputType').value;
    ...
    if (inputType === 'CONSTANT'){
      ...
      this.grpcServerNodeConfigFormGroup.get('param').patchValue([], {emitEvent: false});
  ...
  setDisabledState(isDisabled: boolean): void {
    ...
    if (this.disabled) {
      this.grpcServerNodeConfigFormGroup.disable({emitEvent: false});
    } else {
      this.grpcServerNodeConfigFormGroup.enable({emitEvent: false});
    }
  }
  ...
  writeValue(value: RuleNodeConfiguration): void {
    ...
    if (this.definedConfigComponent) {
      ...
      this.grpcServerNodeConfigFormGroup.patchValue({
      ...
      this.changeSubscription = this.grpcServerNodeConfigFormGroup.get('param').valueChanges.subscribe(
        (configuration: any) => {
      ...
      this.changeSubscription = this.grpcServerNodeConfigFormGroup.get('branchparam').valueChanges.subscribe(
        (configuration: any) => {
      ...
      this.changeSubscription = this.grpcServerNodeConfigFormGroup.get('constant').valueChanges.subscribe(
        (configuration: any) => {
      ...
      this.changeSubscription = this.grpcServerNodeConfigFormGroup.get('property').valueChanges.subscribe(
        (configuration: any) => {
  ...
  private updateModel(configuration: RuleNodeConfiguration) {
    ...
    if (this.definedConfigComponent || this.grpcServerNodeConfigFormGroup.valid) {
